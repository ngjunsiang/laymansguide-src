...
When was the last time you remembered entering an IP address into the address bar in your browser? (If you’re a tinkerer, maybe the last time you tried to configure your router through a web browser.) Almost all the time, we enter web addresses that use **domain names**, not IP addresses. Typical domain names: www.facebook.com, www.google.com, www.instagram.com. The `https` part in front of addresses like https://www.facebook.com is the protocol; a web browser would use that to determine which set of rules to use in processing the data received. I first introduced the idea of a protocol in [Issue 7](https://buttondown.email/laymansguide/archive/lmg-issue-7-what-is-http/).

Here’s something that might be new to some of you: www.facebook.com is not the domain name. facebook.com is the domain name.

That’s how it was conceptualised in the 1980s, where people already realised that memorising names is much easier than memorising IP addresses. The original specifications for the Domain Name System were published in 1983, making this system less than 40 years old.
...
1. The country code top-level domain (ccTLD), based on two-character territory codes. Example: .sg, .my, .us, .ru
2. The generic top-level domains – .gov, .edu, .com, .mil, .org, .net, .int

A web address must have a generic top-level domain describing its category, and optionally may also have a country code top-level domain to signify its associated country. This helps people to identify the category/purpose and origin of the website.
...
What do these organisations do with their huge list of domain names? They put them in databases, of course. If you want to find out who registered mydesireddomainname.net, you would send a request to a WHOIS server (that is not an acronym) and it would give you the registrants information. Here, [give it a try](https://whois.icann.org/en). Just key in any domain name you know. I’ll wait.
